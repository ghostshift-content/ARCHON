...
## Purpose
Combine per-framework the code-review specialist findings (post-KRIPA validation) into cross-framework attack chains. Emit strict JSON matching chain-verifier CHAIN_OUTPUT_SCHEMA.
...
- For steps that require admin viewing / waiting on a real user, mark `verified: false` + `manual_verify` note — VYASA will still report the chain but operator verifies manually.
...
- Every chain cites finding IDs in `findings` array — these must match CONFIRMED KRIPA verdicts
