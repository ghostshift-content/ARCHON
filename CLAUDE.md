...
> **🗂️ Folder layout (restructured 2026-06-08):** Personas now live at `squads/<sq>/agents/<name>/` (universals at `_universal/agents/<name>/`), runtime state is evicted to `var/state/agents/<name>/`. **NEVER hardcode a persona path** — use `paths.js` (`agentPaths.soulPath(name)`, `skillsDir`, `personaState`, `lessonsPath`…), enforced by GATE-121. Layout is driven by `layout.config.json` (personaMode/stateMode) + `ownership.json`, read at runtime by both daemon and dashboard (`mission-control/lib/agent-paths.ts` mirrors paths.js). Full map: `STRUCTURE.md`. Design: `docs/research/2026-06-07-archon-restructure-design.md`.

## What this is

ARCHON is a Node.js framework that runs LLM-powered specialist agents across 7 domain squads:

| Squad | Leader | Specialists | Domain |
|---|---|---|---|
| stocks | CHANAKYA | the veteran, the analyst, LAKSHMI, NARAD, SURYA, VAYU, SHAKUNI, VIDURA, VISHNU | Equity research |
| pentest | ATLAS | SCOUT, RANGER, RELAY, DRILL, VIPER, GATEWAY, WARDEN, VAULT, TRACER, KEYRING, FORGE, LEDGER, DECOY, SPECTRE, SENTRY | Web security |
| cloud-security | VARUNA | AGNI, MITRA, SOMA, KUBERA | AWS/Azure/GCP audit |
| network-pentest | SHALYA | INDRA, GHATOTKACHA | Network/AD |
| code-review | CURATOR | MARSHAL, SIPHON, CIPHER, QUILL, BEACON, BREAKER, +PROBER runtime | White-box review |
| red-team | PARASHURAMA | (varies) | Adversarial |
| ai-security | (lead TBD) | MAYA (scaffolded) | LLM/prompt injection |

Plus universal agents (live in `_universal/agents/`): **NEXUS** (event router, no dir), **AUDITOR** (independent verifier), **ARBITER** (confidence-calibrated judge), **SCRIBE** (final reporter), **COMMAND** (main-squad lead). Note: **SENTRY** is a pentest specialist per code (`squadId=pentest-squad`), not universal — listed under pentest above.
...
| `chain-verifier.js` | Phase 3.6 semantic chain validation (curl + status-code). GATE-120 (2026-06-06): curl results annotate VALIDATED-FINDINGS (`chain_verified`/`chain_evidence`) so ARBITER Stage C judges on real HTTP evidence |
...
| 0.5 | TRACER surface discovery / WAF detect | dispatchPentestParallel | pentest |
| 0.7 | Complexity scoring (model routing input) | dispatchPentestParallel | pentest |
| 1 | Recon (SCOUT, RANGER — or analysts for stocks) | per-squad dispatcher | varies |
...
| 2 | Specialist parallel execution (RELAY/VIPER/DRILL/GATEWAY/...) — adaptive 2-wave batching (GATE-112) | per-squad dispatcher | varies |
...
| 3 | AUDITOR independent verifier | per-squad | pentest, cloud, network, code-review |
| 3.05 | AUDITOR verdict bridge → VALIDATED-FINDINGS.jsonl | per-squad | same as 3 |
...
| 3.9 | ARBITER judge w/ promotion gate — 3-judge majority vote on High/Critical (GATE-117), chain-evidence in Stage C (GATE-120) | same | all |
| 4 | SCRIBE final report (cleanReportForPublish) | same | all |
| 5 | Grading + report blocking (SENTRY) | same | all |
...
| **Promotion/Judge gates** | 67, 69 | ARBITER + extractTargetUrl priority |
...
Use `selectBestDossierFile(scanDirs, taskId, leader, cutoffMs, { canonicalSpec })` from `agents/dossier-selector.js`. The published report must be the **canonical author's** file — resolved by ROLE via `canonicalReportRole(squad)`: analysis squads (stocks) → the leader (CHANAKYA); security squads → the reporter (SCRIBE), **NOT** their leader (ATLAS/VARUNA/… write side artifacts). Priority (highest first):
1. **P0a Sidecar** — `/root/intel/reports/<taskId>.canonical` JSON pointer (unspoofable by filename; makes re-grades idempotent)
2. **P0b Marker** — `<!-- ARCHON-CANONICAL taskId=<taskId> … -->` on line 1 of the file
...
| Phase 3.9 ARBITER + promotion | LIVE | Medium→High promotion via 4-stage rubric |
...
- `/root/agents/docs/research/2026-06-03-archon-START-HERE.md` — **next-gen architecture** (design): inside-CC, dynamic-workflows + Agent SDK, domain-agnostic kernel, gated self-improvement loop. Canonical: `…-THE-FRAMEWORK.md`.
