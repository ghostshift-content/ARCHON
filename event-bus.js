...
function getStocksAnalysts() {
  const dynamic = configCache.getAgentsByRole('stocks-squad', 'analyst')
  return dynamic.length > 0 ? dynamic : FALLBACK_STOCKS_ANALYSTS
}

function getStocksChallengers() {
  const dynamic = configCache.getAgentsByRole('stocks-squad', 'challenger')
...
async function dispatchVerification(taskId, taskTitle, squad, originalAgent, findings) {
  ...
  const skillMap = {
    ...
    'stocks': 'stocks-verification',
    'red-team': 'red-team-verification',
    'cloud-security': 'cloud-security-verification',
    'network-pentest': 'network-verification',
...
function mergeChanakyaFinalDossier(taskId, projectId, squad, phaseStartTs) {
  try {
    const startMs = phaseStartTs ? new Date(phaseStartTs).getTime() : 0
    // (2026-04-20 universal fix) Use the squad's configured leader (not
    // hardcoded CHANAKYA) so new analysis-style squads work without edits.
    const leader = String(getConfiguredSquadLeader(squad) || 'chanakya').toUpperCase()
    // Fast path via per-task log — avoids scanning the 500MB global log.
    const allEntries = readTaskActivity(taskId)
      .filter(Boolean)
      .filter(e => (e.agent || '').toUpperCase() === leader)
      .filter(e => (e.type || '') !== 'final-dossier-merged')
      .filter(e => !String(e.action || '').startsWith('💰 Cost:'))
      .filter(e => !String(e.action || '').includes('Quality Score'))
      .filter(e => {
        const tsMs = new Date(e.ts || 0).getTime()
        return Number.isFinite(tsMs) && tsMs >= startMs
      })
      .sort((a, b) => new Date(a.ts || 0).getTime() - new Date(b.ts || 0).getTime())

    if (allEntries.length === 0) return null

    // Prefer the Final Dossier entry (golden template) — the last substantive entry from Phase 4
    // Skip intermediate synthesis (CIO Synthesis from Phase 2) to avoid polluting the report
    const finalDossierEntry = allEntries
      .filter(e => String(e.action || '').includes('Final Dossier'))
      .slice(-1)[0]  // take the last Final Dossier entry

    // If a clean Final Dossier entry exists, use only that — it's the complete report
    let mergedDetails
    if (finalDossierEntry && String(finalDossierEntry.details || '').length > 1000) {
      mergedDetails = String(finalDossierEntry.details || '').trim()
    } else {
      // Fallback: merge all entries (old behaviour)
      const entries = allEntries
      mergedDetails = entries
        .map((e, idx) => {
          const title = String(e.action || '').trim() || `Section ${idx + 1}`
          const body = String(e.details || '').trim()
    ...
    if (!mergedDetails.trim()) return null

    const sectionCount = finalDossierEntry ? 1 : allEntries.length
    ...
    log(`⚠️ Failed to merge CHANAKYA final dossier for task ${taskId}: ${e.message}`)
    return null
  }
}
...
function buildAnalystPrompt(agentName, taskTitle, taskId, projectId, squad, goalContext) {
  ...
  // Smart memory ranking for stock analysts
  let smartLessonsCtx = ''
  try {
    const stockName = taskTitle.replace(/\s*stock\s*scan.*/i, '').trim()
    const result = memoryRanker.getRelevantLessons(agentName, squad, {
      targetDomain: stockName,
      agentName: agentName,
      squad: squad,
      keywords: taskTitle.toLowerCase().split(/[\s—]+/).filter(w => w.length > 3)
    }, 5)
    if (result.count > 0) {
      smartLessonsCtx = result.text
      log(`🧠 Smart memory: ${result.count} lessons for ${agentUpper} on ${stockName}`)
    }
  } catch {}
  const gates = getSquadGates(squad)
  // Read peer findings from analysts who went before — group-chat-style cross-refinement
  // (2026-06-09): confidence-ranked (high→low) + a "build-on / challenge / fill-gaps" framing
  // instead of a raw last-15 dump, so a later analyst actively refines off peers rather than
  // working in isolation. Inspired by Microsoft Agent Framework's group-chat iterative refinement.
  const liveFindingsFile = `${agentPaths.INTEL_ROOT}/live-findings-${taskId}.jsonl`
  let liveFindings = ''
  try {
    if (fs.existsSync(liveFindingsFile)) {
      const parsed = fs.readFileSync(liveFindingsFile, 'utf-8').trim().split('\n').filter(Boolean)
        .map(l => { try { return JSON.parse(l) } catch { return null } })
        .filter(f => f && f.finding && String(f.agent || '').toUpperCase() !== agentUpper) // not my own
      if (parsed.length > 0) {
        const rank = c => ({ high: 0, medium: 1, low: 2 }[String(c).toLowerCase()] ?? 3)
        const ranked = parsed.sort((a, b) => rank(a.confidence) - rank(b.confidence)).slice(0, 20)
        liveFindings = `\n## PEER FINDINGS (analysts before you — BUILD ON, CHALLENGE, or fill the gaps; do NOT just repeat):\n` +
          ranked.map(f => `- [${f.agent}·${f.signal || '?'}·${f.confidence || '?'}] ${f.finding}`).join('\n') + `\n`
      }
    }
  } catch {}
  return `You are ${agentUpper}. Task: ${taskTitle}. TaskID: ${taskId}. Squad: ${squad}.
${goalLine}${gates}${feedbackCtx}${liveFindings}${smartLessonsCtx}
...
## CROSS-COLLABORATION: Share key findings for other analysts
When you discover something important (key metric, risk factor, catalyst, red flag), share it:
echo '{"agent":"${agentUpper}","type":"TYPE","finding":"WHAT_YOU_FOUND","signal":"bullish|bearish|neutral","confidence":"high|medium|low"}' >> ${agentPaths.INTEL_ROOT}/live-findings-${taskId}.jsonl
...
function buildSynthesizerPrompt(taskTitle, taskId, projectId, squad, phase, goalContext) {
  const goalLine = goalContext ? `Goal: ${goalContext}\n` : ''
  const feedbackCtx = getDisprovenContext(squad) + getSquadLessons(squad)
  const gates = getSquadGates(squad)
  ...
  const verbosityHint = phase === 'final'
    ...
    : ''
  return `You are CHANAKYA, CIO of ${squad}. Phase: ${phase}. Task: ${taskTitle}. TaskID: ${taskId}.
${goalLine}${verbosityHint}${gates}${feedbackCtx}
...
function buildChallengerPrompt(agentName, taskTitle, taskId, projectId, squad, goalContext) {
  const agentUpper = agentName.toUpperCase()
  const agentLower = agentName.toLowerCase()
  const goalLine = goalContext ? `Goal: ${goalContext}\n` : ''
  const feedbackCtx = getDisprovenContext(squad) + getSquadLessons(squad)
  const gates = getSquadGates(squad)
  return `You are ${agentUpper}, challenger in ${squad}. Task: ${taskTitle}. TaskID: ${taskId}.
${goalLine}${gates}${feedbackCtx}
Read your skill: cat ${agentPaths.skillsDir(agentLower)}/*/SKILL.md
Read references: cat ${agentPaths.skillsDir(agentLower)}/*/references/*.md 2>/dev/null
...
function buildCloudSpecialistPrompt(agentName, taskTitle, taskId, projectId, squad, goalContext, accountId, cloud, regions) {
  const agentU = agentName.toUpperCase()
  const scrubbedGoal = scrubBaselineFromGoal(goalContext)
  const goalLine = scrubbedGoal ? `Goal: ${scrubbedGoal}\n` : ''
  const feedbackCtx = getDisprovenContext(squad, accountId) + getSquadLessons(squad, accountId) + getFreshEyesNotice(accountId)
  const cloudContext = {
    aws: `AWS account: ${accountId}, regions: ${(regions || ['us-east-1']).join(',')}\nPrimary CLI: aws\nNuclei templates: ~/nuclei-templates/cloud/aws/\nPrefer prowler for CSPM scan.`,
    gcp: `GCP project: ${accountId}\nPrimary CLI: gcloud\nPrefer scout-suite + prowler.`,
    azure: `Azure subscription: ${accountId}\nPrimary CLI: az\nPrefer scout-suite + prowler.`,
    k8s: `K8s cluster context (kubectl current-context)\nPrefer kubescape + kube-bench + kubectl audit.`,
  }[cloud] || `Cloud: ${cloud} (unknown — enumerate via available CLI)`
  return `You are ${agentU}, cloud security specialist for the ${squad} squad.
${goalLine}
## Cloud Context
${cloudContext}
...
## Inputs
- Authenticated CLI access via host identity (no need to re-auth)
- Nuclei cloud templates at ~/nuclei-templates/cloud/${cloud}/
- Tool availability: aws, gcloud, az, kubectl, prowler, scoutsuite (check which exist)

## Output
Write findings to: ${agentPaths.INTEL_ROOT}/cloud/findings/${taskId}/${agentName.toLowerCase()}-${cloud}.jsonl
Format (one JSON per line):
  {"id":"${agentU}-NNN","severity":"Critical|High|Medium|Low|Info","title":"...","resource":"arn:aws:..."|"projects/...","description":"...","evidence":"exact command + output","remediation":"..."}

${MUST_GATES}${feedbackCtx}
${A2A_HANDOFF_SECTION}
## Must Not
- Modify any resource (read-only posture)
- Claim findings without evidence (exact command + actual output required)
- Hallucinate regions/services you haven't enumerated
...
## Task
Emit strict JSON chains per cloud-chain-analysis skill. Max 8 chains.
Every chain MUST combine findings from ≥2 DIFFERENT specialists.

## Verification Realism
- Use status-code checks ("HTTP 200") not body substrings
- curl commands only (chain-verifier blocks anything else)
- Cloud-side curl: use presigned URLs or metadata endpoints where possible

Output MUST validate against CHAIN_OUTPUT_SCHEMA. Empty chains array if no real chains exist.`
}

// ── Build network specialist prompt (INDRA/GHATOTKACHA) — 2026-04-23 ──
function buildNetworkSpecialistPrompt(agentName, taskTitle, taskId, projectId, squad, goalContext, meta, hostsFile, kind) {
  const agentU = agentName.toUpperCase()
  const scrubbedGoal = scrubBaselineFromGoal(goalContext)
  const goalLine = scrubbedGoal ? `Goal: ${scrubbedGoal}\n` : ''
  const scopeLine = `CIDR: ${meta.cidr}${meta.dcIp ? ', DC IP: ' + meta.dcIp : ''}${meta.domain ? ', Domain: ' + meta.domain : ''}`
  const authMode = meta.creds ? `Authenticated (user=${meta.creds.user})` : 'Unauthenticated / null-session'
  const kindHint = kind === 'ad'
    ? 'Focus: Active Directory, LDAP, Kerberos, SMB signing, RID brute, SPN enum, ASREPRoast enum. DETECTION ONLY — no ticket cracking, no relay, no lateral move.'
    : 'Focus: protocol services — SMB shares, FTP anon, SSH banners, SNMP community strings, Redis anon, RDP exposure, IPMI. DETECTION ONLY.'
  return `You are ${agentU} for the ${squad} squad.
${goalLine}
## Scope & Auth
${scopeLine}
Auth mode: ${authMode}
Live hosts file: ${hostsFile}

## Kind
${kindHint}

## Detection-Only Constraints (MANDATORY)
- ❌ NO exploitation (no PtH, no Kerberoast cracking, no lateral move, no data modify)
- ❌ NO credential cracking (hash save allowed — cracking NOT allowed)
- ✅ Enumerate, probe, classify, log — then STOP
- ✅ Every finding must include exact command + exact output

## Tools available
nmap, nxc (netexec), impacket-* (GetUserSPNs, GetNPUsers, secretsdump — enum only, NO execution), bloodhound-python, responder (LOG-ONLY), kerbrute userenum (NOT passwordspray), certipy (find only, NOT req/auth), mitm6 (--only-dns, observe only)

## Output
Write findings to: ${agentPaths.INTEL_ROOT}/network/findings/${taskId}/${agentName.toLowerCase()}-${kind}.jsonl
Format: {"id":"${agentU}-NNN","severity":"...","title":"...","host":"10.0.0.X","service":"smb|ldap|kerberos|ssh|...","description":"...","evidence":"<command>\n<output>","remediation":"..."}
...
Every chain combines findings from BOTH INDRA (AD) and GHATOTKACHA (services), OR 2+ distinct services.
Max 8 chains. Detection-mode: chains describe attack PATHS but do not execute them.

## Verification Realism
Chain-verifier only runs curl. For network-layer attacks, emit chains with a curl-based indirect check (e.g. curl to web-facing service on target port) OR mark verified:false with manual_verify note.
...
## Output
Write to: ${agentPaths.INTEL_ROOT}/network/FINAL-REPORT-${taskId}.md
Structure: Executive Summary, Scope (CIDR: ${meta.cidr}, detection-mode), Host Inventory, Attack Chains, Findings by Host + Service, Remediation Roadmap, Appendix (enumeration methodology, tools used).

## Must Not
- Include unverified findings as confirmed
- Expose internal agent names (use cleanReportForPublish titles)
- Imply exploitation occurred — detection-mode only
...
## Output
Write to: ${agentPaths.INTEL_ROOT}/cloud/FINAL-REPORT-${taskId}.md
Structure: Executive Summary, Scope & Methodology, Verified Attack Chains (FIRST), Context Inventory (REQUIRED — see below), Findings by Cloud (AWS/GCP/Azure/K8s sections), Remediation Roadmap.

## Context Inventory (REQUIRED section — borrowed from ARCHON methodology 2026-04-23)
Output EXACTLY one markdown table covering every cloud resource → access-path → defense chain the squad probed:
| # | Resource (ARN/project/sub/cluster) | Exposure (public/internal/VPC-only) | Access control (IAM policy/RBAC/NSG/SG/bucket-policy) | Identity surface (who can reach) | Verdict (protected/vulnerable/partial) |
Every row with Verdict != "protected" must cite a finding ID from the Findings section. Protected rows still belong as defensive evidence. The Context Inventory is the auditor's map of the blast radius.

## Must Not
- Include unverified findings as "confirmed"
- Invent any resource ARN or resource ID
- Use internal agent names (AGNI/MITRA/etc.) — use professional titles per cleanReportForPublish
- Omit the Context Inventory table — this is the machine-parseable attack-surface summary
...
async function dispatchStocksParallel(dispatch) {
  const { taskId, taskTitle, squad, projectId, model: modelOverride, goal: taskGoal } = dispatch
  let totalCost = 0
  let budgetExceeded = false
  const allCosts = []

  // Dynamic agent lists (resolved once per dispatch for consistency)
  const STOCKS_ANALYSTS = getStocksAnalysts()
  const STOCKS_CHALLENGERS = getStocksChallengers()
  
  // (2026-04-20 fix) Previously the stocks pipeline hardcoded FALLBACK_CHAIN
  // starting with claude-sonnet-4-6, bypassing the modelRouter family system.
  // That meant stock_leader (CHANAKYA) NEVER got its configured `powerful`
  // family (Opus 4.7) despite model-config.json mapping it there.
  // Now: align with the pentest path — if dispatch specified a model, use it;
  // otherwise return null and let spawnAgent's modelRouter.getModelForAgent()
  // pick based on role + family (stock_leader → powerful → Opus 4.7 xhigh).
  function getAvailableModel() {
    if (modelOverride) {
      const check = quotaManager.canDispatch(modelOverride)
      if (check.allowed) return modelOverride
      // Rate-limited? Fall back to balanced sonnet / fast haiku.
      const fallback = modelOverride.includes('sonnet')
        ? 'anthropic/claude-haiku-4-5'
        : 'anthropic/claude-sonnet-4-6'
      if (quotaManager.canDispatch(fallback).allowed) {
        log(`   ↪ Stocks model fallback: ${modelOverride} → ${fallback}`)
        return fallback
      }
      return modelOverride
    }
    // No explicit override → defer to modelRouter via spawnAgent.
    return null
  }
  
  // Spawn agent with retry on rate limit or stuck/killed (universal)
  async function spawnWithRetry(agentName, prompt, suffix, maxRetries = 1) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      const model = getAvailableModel()
      if (attempt > 0) {
        log(`   ♻️ Retry ${attempt}/${maxRetries} for ${agentName.toUpperCase()} (model: ${model})`)
      }
      const result = await spawnAgent(agentName, taskId, prompt, suffix, model)

      // Check if rate limited
      if (result.code !== 0 && quotaManager.isRateLimitError(result.output || result.error || '')) {
        if (attempt < maxRetries) {
          const waitMs = Math.min((attempt + 1) * 120000, 600000)
          log(`   ⏸️ ${agentName.toUpperCase()} rate-limited — waiting ${waitMs/60000} min before retry`)
          await new Promise(r => setTimeout(r, waitMs))
      ...
      // Killed by watchdog (SIGTERM=143, SIGKILL=137) or crashed with no output → retry.
      // BUT: if the task was cancelled by user (not watchdog), the SIGTERM came from us —
      // don't retry, just exit. _isTaskCancelled reads tasks.json once to distinguish.
      const wasKilled = result.code === 143 || result.code === 137 || result.code === null
      const noOutput = !(result.output || '').trim() || (result.output || '').length < 50
      if (wasKilled && noOutput && attempt < maxRetries) {
        if (_isTaskCancelled(taskId)) {
  ...
  try {
    // Model override note: per-task model override requires manual gateway restart.
    // The model selector in UI is for agent-level config changes.
    if (modelOverride) {
      log(`ℹ️ Model override requested (${modelOverride}) but requires gateway restart. Using current default model.`)
    }

    // ── Phase Resume: check if we can skip completed phases ──
    const completedPhases = dispatch.completedPhases || []
    const isResume = dispatch.resumeFrom === 'resume' && completedPhases.length > 0
    if (isResume) {
      log(`⏩ RESUME MODE: Skipping completed phases: [${completedPhases.join(', ')}]`)
      logEvent('PHASE_RESUME', { taskId, completedPhases })
    }

    const shouldSkipPhase = (phase) => isResume && completedPhases.includes(phase)
    ...
    if (shouldSkipPhase('analysts') || shouldSkipPhase('analysts-1a')) {
      log(`⏩ Skipping Phase 1 (analysts) — already completed`)
      updateProgress(45, 'Phase 1 skipped — resuming from later phase')
    } else {
    ...
    updateProgress(8, 'Phase 1a: First 2 analysts running')
    
    const batch1Results = await Promise.all(batch1.map(agent => {
      const prompt = buildAnalystPrompt(agent, taskTitle, taskId, projectId || '', squad, dispatch.goal || '')
      return spawnWithRetry(agent, prompt, undefined)
    }))
    trackCosts(batch1Results)
    
    log(`🔄 Phase 1b: Dispatching batch 2 (${batch2.map(a=>a.toUpperCase()).join(', ')})`)
    logEvent('PHASE_START', { taskId, phase: 'analysts-1b', agents: batch2.map(a => a.toUpperCase()) })
    updateProgress(18, 'Phase 1b: Next 2 analysts running')
    
    const batch2Results = await Promise.all(batch2.map(agent => {
      const prompt = buildAnalystPrompt(agent, taskTitle, taskId, projectId || '', squad, dispatch.goal || '')
      return spawnWithRetry(agent, prompt, undefined)
    }))
    trackCosts(batch2Results)
    
    let batch3Results = []
    if (batch3.length) {
      log(`🔄 Phase 1c: Dispatching batch 3 (${batch3.map(a=>a.toUpperCase()).join(', ')})`)
      logEvent('PHASE_START', { taskId, phase: 'analysts-1c', agents: batch3.map(a => a.toUpperCase()) })
      updateProgress(28, 'Phase 1c: extra analysts running')
      batch3Results = await Promise.all(batch3.map(agent => {
        const prompt = buildAnalystPrompt(agent, taskTitle, taskId, projectId || '', squad, dispatch.goal || '')
        return spawnWithRetry(agent, prompt, undefined)
      }))
      trackCosts(batch3Results)
    }
    
    const analystResults = [...batch1Results, ...batch2Results, ...batch3Results]
    // Costs already tracked per batch above — don't double-count
    ...
    updateProgress(35, 'Phase 1 complete — analysts done')
    ...
    // Spotcheck: verify all analysts produced output
    const analystOutput = {}
    const taskEntries = getTaskActivity(taskId)
    for (const analyst of STOCKS_ANALYSTS) {
      const entries = taskEntries.split('\n').filter(l => l.includes(analyst.toUpperCase()) && !l.includes('Cost:'))
      analystOutput[analyst] = entries.length
    }
    const emptyAnalysts = STOCKS_ANALYSTS.filter(a => (analystOutput[a] || 0) === 0)
    if (emptyAnalysts.length > 0) {
      log(`⚠️ Spotcheck: ${emptyAnalysts.length} analysts produced no output: ${emptyAnalysts.join(', ')}`)
      for (const agent of emptyAnalysts) {
        log(`♻️ Re-running ${agent.toUpperCase()} (produced no output)`)
        const prompt = buildAnalystPrompt(agent, taskTitle, taskId, projectId || '', squad, dispatch.goal || '')
        const retryResult = await spawnWithRetry(agent, prompt, `task-${taskId}-${agent}-retry`)
        trackCosts([retryResult])
      }
    }
    } // end Phase 1 skip check

    // ── PHASE 1.5: Build Stock Analysis Graph (Level 3) ──
    try {
      const stockName = taskTitle.replace(/\s*stock\s*scan.*/i, '').replace(/\s*—.*/,'').trim()
      const stockGraph = attackGraph.buildGraphFromStockFindings(taskId, stockName)
      if (stockGraph && stockGraph.nodes.size > 0) {
        log(`🕸️ Stock Graph: ${stockGraph.nodes.size} nodes, ${stockGraph.edges.length} edges for ${stockName}`)
      }
    } catch (e) { log(`⚠️ Stock graph error: ${e.message}`) }

    // ── PHASE 2: CHANAKYA Initial Synthesis ──
    if (shouldSkipPhase('synthesis') || shouldSkipPhase('synthesis-2')) {
      log(`⏩ Skipping Phase 2 (synthesis) — already completed`)
      updateProgress(55, 'Phase 2 skipped')
    } else {
    ...
    updateProgress(45, 'Phase 2: CHANAKYA synthesizing')
    
    const synthPrompt = buildSynthesizerPrompt(taskTitle, taskId, projectId || '', squad, 'initial', dispatch.goal || '')
    const synthResult = await spawnWithRetry('chanakya', synthPrompt, `task-${taskId}-chanakya-synth`)
    trackCosts([synthResult])
    
    log(`✅ Phase 2 complete: CHANAKYA synthesis done`)
    logEvent('PHASE_DONE', { taskId, phase: 'synthesis' })
    updateProgress(55, 'Phase 2 complete — synthesis done')
    
    } // end Phase 2 skip check

    // ── PHASE 3: Challengers in Parallel (skip if budget exceeded or already done) ──
    if (shouldSkipPhase('challengers') || shouldSkipPhase('challengers-3')) {
      log(`⏩ Skipping Phase 3 (challengers) — already completed`)
    ...
    updateProgress(60, 'Phase 3: Challengers running')
    
    const challengerPromises = STOCKS_CHALLENGERS.map(agent => {
      const prompt = buildChallengerPrompt(agent, taskTitle, taskId, projectId || '', squad, dispatch.goal || '')
      return spawnWithRetry(agent, prompt, undefined)
    })
    
    const challengerResults = await Promise.all(challengerPromises)
    trackCosts(challengerResults)
    
    log(`✅ Phase 3 complete: All challengers done`)
    logEvent('PHASE_DONE', { taskId, phase: 'challengers' })
    updateProgress(75, 'Phase 3 complete — challengers done')
    } // end budget check for Phase 3
    ...
    updateProgress(80, 'Phase 4: Final dossier')

    const phase4StartTs = new Date().toISOString()
    try {
      const finalPrompt = buildSynthesizerPrompt(taskTitle, taskId, projectId || '', squad, 'final', dispatch.goal || '')
      const finalResult = await spawnWithRetry('chanakya', finalPrompt, `task-${taskId}-chanakya-final`)
      trackCosts([finalResult])

      // Save agent's actual output as the report (not just the activity log placeholder)
      const savedReport = saveAgentReport(taskId, projectId || '', squad, finalResult.output || '')
      if (savedReport) {
        log(`📄 Final report saved (${(savedReport.length / 1024).toFixed(1)}KB) → ${savedReport.reportPath}`)
      } else {
        // Fallback: check for dossier files CHANAKYA may have written before dying
        const mergedDossier = mergeChanakyaFinalDossier(taskId, projectId || '', squad, phase4StartTs)
        if (mergedDossier) {
          log(`🧩 CHANAKYA final dossier merged from activity log (${mergedDossier.sectionCount} sections)`)
        }
      }
    } catch (phase4Err) {
      log(`⚠️ Phase 4 CHANAKYA failed: ${phase4Err.message} — attempting dossier recovery`)
      // Even if CHANAKYA crashed, try to save whatever we can
      try {
        const mergedDossier = mergeChanakyaFinalDossier(taskId, projectId || '', squad, phase4StartTs)
        if (mergedDossier) {
          log(`🧩 Recovered dossier from activity log (${mergedDossier.sectionCount} sections)`)
        } else {
          // Last resort: save a stub report indicating Phase 1-3 data is available
          const reportPath = path.join(INTEL_DIR, 'reports', `${taskId}.md`)
          fs.mkdirSync(path.join(INTEL_DIR, 'reports'), { recursive: true })
          fs.writeFileSync(reportPath, `# ${taskTitle}\n\n⚠️ Final dossier generation failed. All analyst data is available in the activity log.\n\nPhase 1: 6/6 analysts completed\nPhase 2: Synthesis completed\nPhase 3: Challengers completed\nPhase 4: Dossier compilation failed — retry needed\n`)
          log(`⚠️ Stub report saved — full dossier needs manual retry`)
        }
      } catch {}
    }

    log(`✅ Phase 4 complete: Final dossier written`)
    logEvent('PHASE_DONE', { taskId, phase: 'final-dossier' })
    updateProgress(90, 'Phase 4 complete — grading')
...
async function smartRetry(taskId, taskTitle, squad, projectId, gradeResult, dispatch, modelOverride) {
  ...
  // Check if the final dossier/report exists
  ...
  // Check if challenger data exists
  const hasChallengerData = ['SHAKUNI', 'VIDURA', 'VISHNU'].some(a =>
    taskActivity.includes(a)
  )
  ...
  if (_gateStyle === 'analysis') {
    // ANALYSIS SMART RETRY LOGIC (stocks etc.)
    if (hasAnalystData && !hasFinalDossier) {
      // Most common case: analysts ran fine, CHANAKYA didn't produce dossier
      // Only re-run CHANAKYA Phase 4 (final dossier)
      // (2026-04-20) Defer to modelRouter — CHANAKYA is stock_leader (powerful
      // family, Opus 4.7 xhigh) and is in deny_family_downgrade_for, so it
      // cannot fall to haiku. The prior "hardcode sonnet to avoid haiku" guard
      // is now structurally enforced by the router; hardcoded sonnet would
      // actually DOWNGRADE from Opus 4.7 for retries. Pass null → router picks.
      log(`  📊 Analysts OK, dossier missing → re-running CHANAKYA Phase 4 only`)

      const model = modelOverride || null
      const goalLine = dispatch.goal ? `Goal: ${dispatch.goal}\n` : ''
...
Write your dossier to ACTIVITY-LOG.jsonl with taskId=${taskId}.
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","agent":"CHANAKYA","action":"Final Dossier [v3.0-golden] — ${taskTitle}","details":"YOUR_FULL_20_SECTION_DOSSIER","taskId":"${taskId}","projectId":"${projectId || ''}","squad":"${squad}"}' >> ${agentPaths.INTEL_ROOT}/ACTIVITY-LOG.jsonl
Execute now.`

      const result = await spawnAgent('chanakya', taskId, prompt, `task-${taskId}-chanakya-smartretry`, model)

      // Save agent's actual output as the report
      let saved = saveAgentReport(taskId, projectId || '', squad, result.output || '')

      // Check activity log for a larger dossier — CHANAKYA often writes the full dossier there
      try {
        const reportPath = path.join(INTEL_DIR, 'reports', `${taskId}.md`)
        const currentSize = saved ? saved.length : (fs.existsSync(reportPath) ? fs.statSync(reportPath).size : 0)
        // Fast path via per-task log — walk newest first.
        const entries = readTaskActivity(taskId)
        for (let i = entries.length - 1; i >= 0; i--) {
          const e = entries[i]
          if (/Final Dossier/i.test(e.action || '') && e.details && e.details.length > currentSize) {
            fs.writeFileSync(reportPath, e.details)
            saved = e.details
            log(`  📄 Found larger dossier in activity log (${(e.details.length / 1024).toFixed(1)}KB vs ${(currentSize / 1024).toFixed(1)}KB stdout)`)
            break
          }
        }
      } catch {}
      ...
      if (saved) {
        log(`  📄 Smart retry report saved (${(saved.length / 1024).toFixed(1)}KB)`)
      } else {
        try {
          const mergedDossier = mergeChanakyaFinalDossier(taskId, projectId || '', squad, new Date().toISOString())
          if (mergedDossier) {
            log(`  🧩 Smart retry dossier merged (${mergedDossier.sectionCount} sections)`)
          }
        } catch {}
      }

      return { retryType: 'chanakya-final-only', cost: result.cost }
    }

    if (!hasAnalystData) {
      // No analyst data at all — need full re-run
      log(`  📊 No analyst data → full re-run needed`)
      return { retryType: 'full-rerun-needed' }
    }

    // Has dossier but low quality — re-run CHANAKYA with explicit gap list
    // (2026-04-20) Defer to modelRouter (CHANAKYA → Opus 4.7 xhigh).
    if (hasFinalDossier) {
      log(`  📊 Dossier exists but incomplete → re-running CHANAKYA with gap instructions`)

      const gapList = failed.map(f => `- ${f.text}`).join('\n')
      const model = modelOverride || null
      const goalLine = dispatch.goal ? `Goal: ${dispatch.goal}\n` : ''

      const prompt = `You are CHANAKYA, CIO of ${squad}. QUALITY RETRY for: ${taskTitle}. TaskID: ${taskId}.
${goalLine}
...
TASK: Write a NEW complete Final Dossier that fixes ALL the gaps listed above. Include ALL 20 sections. Do NOT skip any section. This dossier REPLACES the previous one.

Write to ACTIVITY-LOG.jsonl with taskId=${taskId}.
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","agent":"CHANAKYA","action":"Final Dossier [v3.0-golden] — ${taskTitle}","details":"YOUR_FULL_20_SECTION_DOSSIER","taskId":"${taskId}","projectId":"${projectId || ''}","squad":"${squad}"}' >> ${agentPaths.INTEL_ROOT}/ACTIVITY-LOG.jsonl
Execute now.`

      const result = await spawnAgent('chanakya', taskId, prompt, `task-${taskId}-chanakya-gapfix`, model)

      // Save agent's actual output as the report
      let savedGap = saveAgentReport(taskId, projectId || '', squad, result.output || '')

      // Check activity log for a larger dossier
      try {
        const reportPath = path.join(INTEL_DIR, 'reports', `${taskId}.md`)
        const currentSize = savedGap ? savedGap.length : (fs.existsSync(reportPath) ? fs.statSync(reportPath).size : 0)
        // Fast path via per-task log — walk newest first.
        const entries = readTaskActivity(taskId)
        for (let i = entries.length - 1; i >= 0; i--) {
          const e = entries[i]
          if (/Final Dossier/i.test(e.action || '') && e.details && e.details.length > currentSize) {
            fs.writeFileSync(reportPath, e.details)
            savedGap = e.details
            log(`  📄 Found larger dossier in activity log (${(e.details.length / 1024).toFixed(1)}KB vs ${(currentSize / 1024).toFixed(1)}KB)`)
            break
          }
        }
      } catch {}
      ...
      if (savedGap) {
        log(`  📄 Gap-fix report saved (${(savedGap.length / 1024).toFixed(1)}KB)`)
      } else {
        try {
          const mergedDossier = mergeChanakyaFinalDossier(taskId, projectId || '', squad, new Date().toISOString())
          if (mergedDossier) {
            log(`  🧩 Gap-fix dossier merged (${mergedDossier.sectionCount} sections)`)
          }
        } catch {}
      }

      return { retryType: 'chanakya-gap-fix', cost: result.cost }
    }
  }
...
async function dispatchToAgent(dispatch) {
  ...
  // (2026-04-23) cloud-security squad routes through the isolated cloud-dispatcher
  // module. Closures (trackCosts, updateProgress, log, logActivity) are passed in
  // via deps since they live in this function's scope.
  if (dispatchType === 'cloud-security') {
    try {
      // Define local closures that mirror the pentest pipeline's trackCosts/updateProgress
      const allCostsLocal = []
      ...
      const cloudDispatcher = freshRequire('./cloud-dispatcher')
      await cloudDispatcher.runCloudSecurity(dispatch, {
      ...
      log(`❌ cloud-security dispatch error: ${e.message}`)
  ...
  // (2026-04-23) network-pentest squad routes through network-dispatcher module.
  // Detection-mode only — no exploitation, no crack, no lateral move.
  if (dispatchType === 'network-pentest') {
    try {
      const allCostsLocal = []
      let totalCostLocal = 0
      const trackCostsLocal = (results) => {
      ...
      const netDispatcher = freshRequire('./network-dispatcher')
      await netDispatcher.runNetworkPentest(dispatch, {
        spawnAgent, trackCosts: trackCostsLocal, updateProgress: updateProgressLocal,
        log, logActivity,
      ...
      log(`❌ network-pentest dispatch error: ${e.message}`)
      try { runningTasks.delete(taskId) } catch {}
      setTimeout(() => processQueue(), 2000)
    }
    return
  ...
  if (dispatchType === 'parallel-challenger') {
    // PARALLEL EXECUTION for stocks-style analysis squads
    try {
      const { totalCost, allCosts } = await dispatchStocksParallel(dispatch)
      
      // Grade the combined output (use chanakya's evals)
      ...
      // Record this run's quality metrics for per-squad baseline tracking.
      // Fail-soft: never breaks the pipeline.
      try {
      ...
      // Run separate LLM grader (independent context, no self-eval bias)
      ...
      if (gradeResult) {
        ...
        // Also write memory for each analyst that participated
        for (const analyst of [...getStocksAnalysts(), ...getStocksChallengers()]) {
          writePostTaskMemory(analyst.toUpperCase(), taskId, taskTitle, squad, gradeResult.passRate, 0, null)
          try { memoryRanker.updateLessonEffectiveness(analyst, taskId, gradeResult) } catch {}
        }
        ...
        if (gradeResult.passRate < 85) {
          ...
          if (retryResult && retryResult.retryType !== 'full-rerun-needed') {
            ...
            if (reGrade) {
              ...
              // (2026-04-20) Sync agent memory with retry grade. Without this,
              // grades.json holds the pre-retry value while tasks.json holds the
              // post-retry value — silent divergence.
              if (reGrade.passRate !== gradeResult.passRate) {
                ...
                for (const analyst of [...getStocksAnalysts(), ...getStocksChallengers()]) {
                  writePostTaskMemory(analyst.toUpperCase(), taskId, taskTitle, squad, reGrade.passRate, 0, null)
                }
      ...
      log(`❌ Stocks dispatch failed: ${e.message}`)
    }
    ...
    if (repairDispatch && repairDispatch.onComplete) {
      try {
        ...
        log(`⚠️ Re-run after repair (stocks) failed: ${e.message}`)
      }
    }

    setTimeout(() => processQueue(), 2000)

  } else if (dispatchType === 'parallel-phases') {
    ...
    if (!hasDispatch && !isRunning) {
      ...
      if (isRecent && t.assignee && !_recoveryBlocked(tid, queue, t)) {
        queue.push({
          ...
          assignee: t.assignee,
          squad: (t.squad || 'stocks') + (t.squad?.includes('-squad') ? '' : '-squad'),
