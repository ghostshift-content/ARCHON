...
// SCRIBE phase. Regression guard for Apr-21 Run 1 hallucinations.
...
try {
  // Scenario 1: thrashing SCOUT wrote 3 bogus completion files — quarantine them.
  ...
  fs.writeFileSync(path.join(PENTEST, 'SCOUT-phantom-output.md'), 'hallucinated data tagged by agent name')
  ...
  const moved1 = quarantineThrashFiles(PENTEST, 'TASK123', 'SCOUT', silentSince)
  ...
  ok('moves SCOUT-tagged output file',  !fs.existsSync(path.join(PENTEST, 'SCOUT-phantom-output.md')))
  ...
  const moved2 = quarantineThrashFiles(PENTEST, 'TASK123', 'SCOUT', silentSince)
  ...
  const moved4 = quarantineThrashFiles(PENTEST, 'TASK999', 'SCOUT', silentSince)
