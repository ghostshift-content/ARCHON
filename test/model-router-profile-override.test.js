...
test('default profile (env unset) — modelRouter normal routing applies', () => {
  withProfile(undefined, () => {
    const result = modelRouter.getModelForAgent('KRISHNA')
...
test('G4_test_sonnet — KRISHNA gets Sonnet via profile override', () => {
  withProfile('G4_test_sonnet', () => {
    const result = modelRouter.getModelForAgent('KRISHNA')
    assert.strictEqual(result.model, 'claude-sonnet-4-6',
      'KRISHNA should be Sonnet under G4_test_sonnet profile')
...
test('G4_test_sonnet — DHARMA falls through to normal routing (not in profile)', () => {
  withProfile('G4_test_sonnet', () => {
    const result = modelRouter.getModelForAgent('DHARMA')
    assert.notStrictEqual(result.family, 'profile_override',
      'DHARMA is not in G4_test_sonnet — should use modelRouter normal routing')
  })
})

test('G4_test_sonnet — VYASA falls through (not in profile)', () => {
  withProfile('G4_test_sonnet', () => {
    const result = modelRouter.getModelForAgent('VYASA')
    assert.notStrictEqual(result.family, 'profile_override')
  })
})
...
test('Profile override is case-insensitive on agent name', () => {
  withProfile('G4_test_sonnet', () => {
    const upper = modelRouter.getModelForAgent('KRISHNA')
    const lower = modelRouter.getModelForAgent('krishna')
