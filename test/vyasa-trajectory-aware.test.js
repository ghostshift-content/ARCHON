...
// test/vyasa-trajectory-aware.test.js
//
// FIX 2 (2026-05-09): VYASA reads trajectory observer signals.
//
// Sprint C.1 (trajectory observer) logs which specialists ran on-track vs
// off-track per task. VYASA did NOT see this signal. This fix injects a
// "SPECIALIST QUALITY SIGNALS" section into the VYASA report prompt so the
// report writer can mention specialist quality and treat off-track findings
// as LESS RELIABLE (not blanket-dismissed).
//
// We test:
//   (a) static — buildVyasaReportPrompt body references the trajectory section
...
test('buildVyasaReportPrompt body mentions SPECIALIST QUALITY SIGNALS section', () => {
...
test('VYASA prompt instructs that off-track findings are LESS RELIABLE (not dismissed)', () => {
  const fnStart = SRC.indexOf('function buildVyasaReportPrompt')
  const fnSlice = SRC.slice(fnStart, fnStart + 14000)
  // Anti-sycophancy: don't tell VYASA to ignore the finding entirely
  assert.match(fnSlice, /less reliable|cross-reference|cross.reference|treat.+(less|with caution)/i,
    'must instruct VYASA to treat off-track evidence as LESS RELIABLE, not dismissed')
})

test('VYASA prompt mentions cross-referencing with judge-verifier or chain-verifier', () => {
  const fnStart = SRC.indexOf('function buildVyasaReportPrompt')
  const fnSlice = SRC.slice(fnStart, fnStart + 14000)
  assert.match(fnSlice, /judge.?verifier|chain.?verifier|JUDGED-FINDINGS|chain.?result/i,
    'must tell VYASA to cross-reference off-track findings with verifiers')
})

// ── Dynamic tests via extracted function ───────────────────────────────────
//
// We extract buildVyasaReportPrompt from event-bus.js into a temp module
...
function buildHelperModule() {
  ...
  const fnVyasa = extractFn(SRC, 'buildVyasaReportPrompt')

  // Stubs needed inside buildVyasaReportPrompt:
...
const MUST_GATES = ''
${fixedFnVyasa}
module.exports = { buildVyasaReportPrompt }
`
  const tmpPath = path.join(os.tmpdir(), `_vyasa-trajectory-helper-${process.pid}.js`)
...
test('setup: extract helper module', () => {
  ...
  buildPrompt = require(helperPath).buildVyasaReportPrompt
})
...
test('dynamic: 5 on-track + 2 off-track for taskId → prompt names off-track agents', () => {
  ...
  try {
    ...
    for (const a of ['NAKUL', 'KARNA', 'BHEEM', 'DHARMA', 'SAHDEV']) {
    ...
    lines.push(JSON.stringify({
      schema_version: SCHEMA_VERSION, task_id: taskId, agent: 'RUDRA',
      verdict: 'off-track', first_failed_dim: 'goal-alignment', reason: 'wrong target'
    }))
    lines.push(JSON.stringify({
      schema_version: SCHEMA_VERSION, task_id: taskId, agent: 'YUYUTSU',
      verdict: 'off-track', first_failed_dim: 'evidence-quality', reason: 'no probes'
    }))
    // 1 unrelated task — must be ignored
    lines.push(JSON.stringify({
      schema_version: SCHEMA_VERSION, task_id: 'other-task', agent: 'ARJUN',
    ...
    assert.match(prompt, /RUDRA/, 'must name off-track agent RUDRA')
    assert.match(prompt, /YUYUTSU/, 'must name off-track agent YUYUTSU')
    ...
    assert.ok(!sectionSlice.includes('ARJUN'),
