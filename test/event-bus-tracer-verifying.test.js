// test/event-bus-tracer-verifying.test.js
//
// Module-level grep tests confirming runtracerAgent wraps the entire tracer
...
test('runtracerAgent is the public wrapper, not the original 360-line body', () => {
  // The wrapper should be SHORT (under ~30 lines)
  const wrapperStart = SRC.indexOf('async function runtracerAgent(target, taskId) {')
  assert.ok(wrapperStart > 0, 'runtracerAgent must be present')
  ...
  assert.ok(wrapperLineCount < 30,
    `wrapper must be small (got ${wrapperLineCount} lines) — the actual body must be in _runtracerAgentInner`)
})

test('runtracerAgent wrapper sets persistCheckpointNow({verifying:true}) before inner call', () => {
  const wrapperStart = SRC.indexOf('async function runtracerAgent(target, taskId) {')
...
test('runtracerAgent wrapper clears verifying flag in finally block', () => {
  const wrapperStart = SRC.indexOf('async function runtracerAgent(target, taskId) {')
  const wrapperBody = SRC.slice(wrapperStart, wrapperStart + 2000)
...
test('runtracerAgent wrapper RE-ASSERTS verifying via setInterval (sticky-flag fix)', () => {
...
test('runtracerAgent wrapper awaits the inner function', () => {
  const wrapperStart = SRC.indexOf('async function runtracerAgent(target, taskId) {')
  const wrapperBody = SRC.slice(wrapperStart, wrapperStart + 2000)
  assert.match(wrapperBody, /\bawait\s+_runtracerAgentInner\s*\(/,
    'wrapper must await _runtracerAgentInner(target, taskId)')
})

test('_runtracerAgentInner contains the actual recon logic', () => {
