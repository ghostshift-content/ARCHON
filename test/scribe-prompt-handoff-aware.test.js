// test/scribe-prompt-handoff-aware.test.js
//
// Sprint C.2 Task 8 (2026-05-10): SCRIBE report prompt scans
...
// by source_finding_id. SCRIBE is told to cite handoff verdicts as
// ADDITIONAL evidence, not replacement for primary finding evidence.
//
// Closes the Sprint C.2 loop: FORGE fires handoff to KUBERA → KUBERA
// verdict in done/ → SCRIBE renders it under the source finding in the
// final report.
//
...
function getscribeBody() {
  const start = SRC.indexOf('function buildscribeReportPrompt')
  assert.ok(start > 0, 'buildscribeReportPrompt not found in event-bus.js')
  const next = SRC.indexOf('\nfunction ', start + 30)
  return SRC.slice(start, next > 0 ? next : start + 12000)
}

function makeTmpBase() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'handoff-scribe-test-'))
...
test('handoff-protocol exports buildCrossSquadCorroborationSection helper', () => {
  assert.equal(typeof handoffProtocol.buildCrossSquadCorroborationSection, 'function',
    'helper must be exported so SCRIBE prompt builder can call it')
})
...
test('SCRIBE prompt body references handoffs/done OR cross-squad OR corroboration', () => {
  const body = getscribeBody()
  assert.match(body, /handoffs\/done|cross-squad|CROSS-SQUAD|corroboration/i,
    'buildscribeReportPrompt must read from /root/intel/handoffs/done/ or mention cross-squad corroboration')
})

test('SCRIBE instructions cite handoff verdicts as ADDITIONAL evidence (anti-sycophancy)', () => {
  const body = getscribeBody()
  // SCRIBE must be told the handoff verdict is ADDITIONAL, not replacement.
  assert.match(body, /additional|supplement|alongside|in addition|cite.+(under|with).+(finding|evidence)/i,
    'SCRIBE must be told handoff verdicts are ADDITIONAL evidence, not replacement for primary finding evidence')
})

test('SCRIBE prompt invokes the helper with the actual taskId', () => {
  const body = getscribeBody()
  ...
  assert.match(body, /buildCrossSquadCorroborationSection\s*\(\s*taskId/,
    'buildscribeReportPrompt must invoke buildCrossSquadCorroborationSection(taskId)')
})
