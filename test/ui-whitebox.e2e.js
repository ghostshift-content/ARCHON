...
;(async () => {
  ...
  try {
    ...
    // preset field hidden until a source dir is entered
    ok('preset field hidden with empty source', await p.$eval('#ptPresetField', el => el.style.display === 'none'))
    await p.fill('#ptUrl', 'http://wb.e2e/')
    await p.fill('#ptSourceDir', SRC); await p.waitForTimeout(200)
    ok('preset field revealed once source entered', await p.$eval('#ptPresetField', el => el.style.display !== 'none'))
