...
;(async () => {
  ...
  try {
    ...
    // White + Black mode → combined engagement (source group hidden until this mode)
    ok('source group hidden in default black-box', await p.$eval('#ptSourceGroup', el => el.style.display === 'none'))
    await p.$eval('#ptMode button[data-v="both"]', el => el.click()); await p.waitForTimeout(200)
    ok('source group shown in White+Black mode', await p.$eval('#ptSourceGroup', el => el.style.display !== 'none'))
