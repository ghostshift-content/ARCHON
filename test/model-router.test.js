...
test('recon agents stay on fast (Haiku) at all complexity levels', () => {
  for (const score of [0, 3, 5, 8]) {
    const arjun = r.getModelForAgent('arjun', { complexityScore: score })
    const rudra = r.getModelForAgent('rudra', { complexityScore: score })
    assert.strictEqual(arjun.family, 'fast', `arjun should stay fast at complexity=${score}, got ${arjun.family}`)
    assert.strictEqual(rudra.family, 'fast', `rudra should stay fast at complexity=${score}, got ${rudra.family}`)
  }
})

test('vuln specialists on balanced at simple target, effort bumps on complex', () => {
  const karna_simple = r.getModelForAgent('karna', { complexityScore: 0 })
  const karna_complex = r.getModelForAgent('karna', { complexityScore: 8 })
  assert.strictEqual(karna_simple.family, 'balanced')
  assert.strictEqual(karna_simple.effort, 'high')
  assert.strictEqual(karna_complex.family, 'balanced')
  assert.strictEqual(karna_complex.effort, 'xhigh', `expected effort bump high→xhigh on complex target`)
})

test('chain_analysis leader (krishna) is on opus-4-8/xhigh', () => {
  const krishna = r.getModelForAgent('krishna', { complexityScore: 0 })
  assert.strictEqual(krishna.family, 'powerful')
  assert.strictEqual(krishna.model, 'claude-opus-4-8')
  assert.strictEqual(krishna.effort, 'xhigh')
})

test('protected agents never downgrade even with override', () => {
  // Write an override trying to downgrade kripa
  const overrides = {
    version: 2,
    overrides: { kripa: { family: 'fast', effort: 'low' } }
  }
  fs.writeFileSync((__roots.INTEL_ROOT + '/agent-model-overrides.json'), JSON.stringify(overrides))
  r.resetCache()

  const kripa = r.getModelForAgent('kripa', { complexityScore: 0 })
  assert.strictEqual(kripa.family, 'balanced', 'kripa family should stay balanced (protected)')
  // Effort override is allowed even on protected agents
  assert.strictEqual(kripa.effort, 'low', 'effort override should still apply')
...
test('rollback mode falls back all agents to balanced/high', () => {
  ...
  for (const agent of ['arjun', 'karna', 'krishna', 'chanakya', 'vyasa']) {
...
test('squad-aware routing: dual-use agents resolve per squad', () => {
  // BHISHMA is in both pentest and stocks. Under squad context, it should
  ...
  const bhishmaStocks = r.getModelForAgent('bhishma', { squad: 'stocks-squad' })
  assert.strictEqual(bhishmaStocks.role, 'stock_analyst', 'BHISHMA in stocks-squad → stock_analyst')
  // (2026-04-21) stock_analyst now on balanced (sonnet) for content depth
  assert.strictEqual(bhishmaStocks.family, 'balanced', 'stock_analyst → balanced family (sonnet, upgraded from haiku)')

  const bhishmaPentest = r.getModelForAgent('bhishma', { squad: 'pentest-squad' })
  assert.strictEqual(bhishmaPentest.role, 'vuln_specialist', 'BHISHMA in pentest-squad → vuln_specialist (no squad override, falls to flat)')
  assert.strictEqual(bhishmaPentest.family, 'balanced', 'vuln_specialist → balanced family (sonnet)')
