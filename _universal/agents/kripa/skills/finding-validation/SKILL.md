# Finding Validation & False Positive Filter — KRIPA 🔍
...
```json
{"agent":"KRIPA","action":"CONFIRMED — [ID]","details":"Q1-Q7 PASSED. Probe: curl -s 'URL' | grep 'EVIDENCE'. Impact: attacker CAN [verb] [object].","taskId":"TASK_ID","squad":"SQUAD"}
```
```json
{"agent":"KRIPA","action":"KILLED — [ID]","details":"FAILED Q[N]: [reason]. Not forwarding to VYASA.","taskId":"TASK_ID","squad":"SQUAD"}
```
```json
{"agent":"KRIPA","action":"CHAIN_REQUIRED — [ID]","details":"Standalone invalid. Routing to chain-builder. Required chain: [B].","taskId":"TASK_ID","squad":"SQUAD"}
```

### Validation Summary
```json
{"agent":"KRIPA","action":"Validation Summary","details":"Total: N | Confirmed: X | Killed: Y | Chain-required: Z | False Positive Rate: P%","taskId":"TASK_ID","squad":"SQUAD"}
```
