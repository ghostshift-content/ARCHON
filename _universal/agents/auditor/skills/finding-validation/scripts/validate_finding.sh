...
OUTPUT_DIR="/tmp/auditor_validation_$$"
mkdir -p "$OUTPUT_DIR"

cleanup() { echo "[*] Evidence saved to: $OUTPUT_DIR (preserve before exit)"; }
trap cleanup EXIT

echo "[AUDITOR] Validating finding: $FINDING_TYPE"
echo "[AUDITOR] Target: $TARGET"
echo ""
...
echo ""
echo "[AUDITOR] Validation complete. Evidence in: $OUTPUT_DIR"
